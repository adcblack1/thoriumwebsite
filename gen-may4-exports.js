...
const cat = require('./src/data/catalyst-db.json');
...
function processContent(html) {
  ...
  // Convert <h2>Into the Valley</h2> → full-bleed image header  
  ...
  // Valley View div/img replacements
  html = html.replace(/<div[^>]*class="vv-header"[^>]*>[\s\S]*?<\/div>/gi, `<div style="padding:16px 0 4px;"><img src="${BASE}/IN%20THE%20VALLEY%20NEWS.png" alt="Into the Valley" style="display:block;width:100%;height:auto;padding:0;" /></div>`);
  html = html.replace(/<img([^>]*?)src="[^"]*into-the-valley[^"]*"([^>]*?)>/gi, `<img src="${BASE}/IN%20THE%20VALLEY%20NEWS.png" alt="Into the Valley" style="display:block;width:100%;height:auto;padding:0;" />`);
  html = html.replace(/width:\s*35%/gi, 'width:100%');
  
  // Convert <ul>/<li> to + bullet paragraphs (must do BEFORE <p> styling)
  ...
  // Style links that don't already have style
  html = html.replace(/<a\s+href="/gi, `<a style="color:#5170ff;text-decoration:none;" href="`);
  
  // Strip CSS classes from any remaining tags
...
<div style="padding:0 15px;text-align:left;">
 <p style="font-family:${SANS};color:#2D2D2D;font-size:16px;line-height:1.6;padding:12px 0;margin:0;"><strong style="font-weight:700;">Good Morning Thorium Valley.</strong> ${introClean.split('\n').join(`</p>\n <p style="font-family:${SANS};color:#2D2D2D;font-size:16px;line-height:1.6;padding:12px 0;margin:0;">`)}</p>
</div>`;
 ...
 <img src="${BASE}/thumbnails/toc-bullet.png" alt="" width="14" height="14" style="width:14px;height:14px;vertical-align:middle;margin-right:8px;">${t}
  ...
  // Secondary TOC
  html += `<div style="padding:14px 15px 0;"><span style="font-family:${SANS};font-size:14px;color:rgba(27,27,27,0.5);margin-right:20px;"><img src="${BASE}/thumbnails/toc-bullet.png" alt="" width="10" height="10" style="width:10px;height:10px;opacity:0.4;vertical-align:middle;margin-right:6px;">What else happened today?</span><span style="font-family:${SANS};font-size:14px;color:rgba(27,27,27,0.5);"><img src="${BASE}/thumbnails/toc-bullet.png" alt="" width="10" height="10" style="width:10px;height:10px;opacity:0.4;vertical-align:middle;margin-right:6px;">What AI tools should I be using?</span></div>`;
 ...
 <div style="padding:12px 0 0;">`;
  ...
  // Articles
  data.article_slugs.forEach(s => {
    html += articleCard(s, campaign);
  });
 ...
 <div style="padding:0 15px;text-align:left;">`;
  ...
  // Sign-off + Footer
  const footerText = isMain ? "That's all for today's Thorium Valley. See you tomorrow." : "That's all for today's Catalyst. See you next time.";
...
// === MAIN NEWSLETTER ===
const may4 = nl.find(n => n.slug === 'may-04-2026');
const mainHtml = buildNewsletter(may4, 'may-04-2026', may4.banner_image_url, true);
fs.writeFileSync('/Users/alexchun/Downloads/Thorium Valley Website/MAY 4 CONTENT/beehiiv-export.html', mainHtml);
console.log('✅ Main export:', Buffer.byteLength(mainHtml), 'bytes');

// === CATALYST NEWSLETTER ===
const may4cat = cat.find(c => c.slug === 'catalyst-may-04-2026');
const catHtml = buildNewsletter(may4cat, 'catalyst-may-04-2026', may4cat.banner_image_url, false);
fs.writeFileSync('/Users/alexchun/Downloads/Thorium Valley Website/MAY 4 CATALYST/beehiiv-export.html', catHtml);
console.log('✅ Catalyst export:', Buffer.byteLength(catHtml), 'bytes');
